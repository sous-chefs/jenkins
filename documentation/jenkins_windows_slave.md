...
Manages Jenkins JNLP slaves on Windows nodes. This is a specialized version of the JNLP slave resource designed for Windows environments.
...
- `remote_fs` - Remote filesystem root for the slave (default: 'C:\\Jenkins')
...
- `user` - Windows user to run the slave service (default: 'Administrator')
...
```ruby
# Create a basic Windows slave
jenkins_windows_slave 'win-builder' do
  remote_fs 'C:/Jenkins'
  user      '.\\Administrator'
...
```ruby
# Create a Windows slave with custom user
jenkins_windows_slave 'win-executor' do
...
```ruby
# Create a Windows slave with environment variables
jenkins_windows_slave 'dotnet-builder' do
...
```ruby
# Create a Windows slave with exclusive usage
jenkins_windows_slave 'win-integration' do
...
```ruby
# Bring a Windows slave back online
jenkins_windows_slave 'win-builder' do
  action :online
end
```

**NOTE** Windows slaves use JNLP for connection, similar to `jenkins_jnlp_slave`, but with Windows-specific service management.

**NOTE** The `password` property is marked as sensitive to prevent credential exposure in logs.

**NOTE** Ensure Java is installed on the Windows slave and accessible in the system PATH, or specify the path via the `java_path` property.
